...
@section('main-content')
  <div class="content">
              ...
              <tbody class="list" id="customers-table-body">
                ...
                <tr class="hover-actions-trigger btn-reveal-trigger position-static">
                  ...
                  <td class="total-spent align-middle white-space-nowrap fw-bold text-center ps-3 text-body-emphasis">
                    <span class="badge badge-phoenix badge-phoenix-success">{{ $vendor_system->unite->code }}</span>
                  </td>
                  <td class="total-spent align-middle white-space-nowrap fw-bold text-center ps-3 text-body-emphasis">
                    <span class="badge badge-phoenix badge-phoenix-success">{{ $vendor_system->quantity }}</span>
                  </td>
                  <td class="total-spent align-middle white-space-nowrap fw-bold text-center ps-3 text-body-emphasis">
                    <span class="badge badge-phoenix badge-phoenix-success">{{ $vendor_system->price_achat }}</span>
                  </td>
                  <td class="total-spent align-middle white-space-nowrap fw-bold text-center ps-3 text-body-emphasis">
                    <span class="badge badge-phoenix badge-phoenix-success">{{ $vendor_system->price_vente }}</span>
                  </td>
                  <td class="total-spent align-middle white-space-nowrap fw-bold text-end ps-3 text-body-emphasis">
                    @if($vendor_system->status == 1) <span class="badge badge-phoenix badge-phoenix-success">Unité de base</span> @else <span class="badge badge-phoenix badge-phoenix-warning">Simple</span> @endif
                  </td>
                  <td class="last-order align-middle white-space-nowrap text-body-tertiary text-end">
                    ...
                    <span class="me-2 text-danger fs-7" data-feather="trash-2" data-bs-toggle="modal" data-bs-target="#DeleteCompte-{{ $vendor_system->id }}" data-fa-transform="shrink-3"></span>
                  </td>
                </tr>
                @endforeach
              </tbody>
    ...
    @foreach($product->vendor_systems as $unite)
    <div class="card-body p-0">
      <div class="p-4 code-to-copy">
        <!-- Right Offcanvas-->
        <div class="offcanvas offcanvas-end" id="offcanvasRight-{{ $unite->id }}" tabindex="-1" aria-labelledby="offcanvasRightLabel">
          ...
          <div class="offcanvas-body">
            <form method="POST" action="{{ route('magasin.produit.updateVendorSystem',$unite->id) }}">
              @csrf
              {{ method_field('PUT') }}
              <input type="hidden" name="product_id" value="{{ $product->id }}">

              <div class="mb-3 text-start">
                <label for="organizerSingle">Sélectionner une unité pour ce produit</label>
                <select class="form-select @error('unite_id') is-invalid @enderror" name="unite_id" id="organizerSingle" data-choices="data-choices" data-options='{"removeItemButton":true,"placeholder":true}'>
                  <option value="">Sélectionner un...</option>
                  @foreach ($unites as $unite)
                    <option value="{{ old('unite_id') ?? $unite->id }}"> {{ $unite->name }}</option>
              ...
              <div class="mb-3 text-start">
                <label class="form-label" for="quantity">Quantité de l'unité</label>
                <input id="quantity" type="quantity" class="form-control @error('quantity') is-invalid @enderror" name="quantity" value="{{ old('quantity') ?? $vendor_system->quantity }}" placeholder="Quantité de l'unité" required autocomplete="quantity">
              ...
              <div class="mb-3 text-start">
                <label class="form-label" for="price_achat">Prix d'achat de l'unité</label>
                <input id="price_achat" type="numeric" class="form-control @error('price_achat') is-invalid @enderror" name="price_achat" value="{{ old('price_achat') ?? $vendor_system->price_achat }}" placeholder="Prix d'achat de l'unité" required autocomplete="price_achat">
                ...
                <div class="mb-3 text-start">
                <label class="form-label" for="price_vente">Prix de vente de l'unité</label>
                <input id="price_vente" type="numeric" class="form-control @error('price_vente') is-invalid @enderror" name="price_vente" value="{{ old('price_vente') ?? $vendor_system->price_vente }}" placeholder="Prix de vente de l'unité" required autocomplete="price_vente">
              ...
              <div class="mb-3 text-start">
                <div class="form-check form-switch">
                  <input class="form-check-input @error('status_unite') is-invalid @enderror" @if($vendor_system->status == 1) checked="" @endif id="status_unite-{{ $vendor_system->id }}" name="status_unite" type="checkbox" value="1" />
                  <label class="form-check-label mt-1" for="status_unite-{{ $vendor_system->id }}">Unité de base</label>
                </div>
                @error('status_unite')
                  <span class="invalid-feedback" role="alert">
                    <strong>{{ $message }}</strong>
    ...
    @foreach($product->vendor_systems as $vendor_system)
      <div class="modal fade" id="DeleteCompte-{{ $vendor_system->id }}" tabindex="-1" style="display: none;" aria-hidden="true">
        <div class="modal-dialog">
          <div class="modal-content">
            ...
            <form action="{{ route('magasin.produit.deleteVendorSystem',$vendor_system->id) }}" method="post">
              @csrf
              {{ method_field('DELETE') }}
              <input type="hidden" name="product_id" value="{{ $product->id }}">
              <div class="modal-body">
                <p class="text-body-tertiary lh-lg mb-3"> Etes vous sure de bien vouloire supprimer l'unité {{$vendor_system->unite->name}} </p>
              </div>
