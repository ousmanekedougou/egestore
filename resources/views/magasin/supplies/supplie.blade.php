          ...
          @foreach ($supplies as $supplie)
            <div class="col-6 col-sm-4 col-md-3 col-lg-2 hover-actions-trigger btn-reveal-trigger">
              <div class="border border-translucent d-flex flex-center rounded-3 mb-3 p-4" style="height:180px;">
                <img class="mw-100 rounded-4" src="@if($supplie->magasin->logo == '') https://ui-avatars.com/api/?name={{ $supplie->magasin->name }} @else {{(Storage::url($supplie->magasin->logo))}} @endif" alt="{{ $supplie->magasin->name }}" /></div>
              ...
              <a class="btn btn-link p-0" href="{{ route('magasin.autres-magasins.show',$supplie->magasin->slug) }}">A propos<span class="fas fa-chevron-right ms-1 fs-10"></span></a>
