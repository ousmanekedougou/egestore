              ...
              @foreach($supplie->supply_orders as $order)
                <tr class="hover-actions-trigger btn-reveal-trigger position-static">
                  ...
                  <td class=" align-middle white-space-nowrap text-body-tertiary fs-9 ps-4 text-end">
                    @if($order->status == 1)
                      @if($order->delivery == 1)
                        <a target="_blank" href="{{ route('magasin.devis.edit',$order->slug) }}" class="me-2 text-success" data-fa-transform="shrink-3"><span data-feather="file-text" ></span></a>
                      @else
                        <span class="me-2 text-info" data-bs-toggle="modal" data-bs-target="#DevisDelivery-{{ $order->id }}" data-feather="truck" data-fa-transform="shrink-3"></span>
                      @endif
                    @elseif($order->status == 2)
                      <span class="me-2 text-success" data-bs-toggle="modal" data-bs-target="#OrderState-{{ $order->id }}" data-feather="edit-3" data-fa-transform="shrink-3"></span>
                    @endif
